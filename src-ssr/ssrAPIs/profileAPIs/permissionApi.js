...
router.post("/add", authorize, async (req, res) => {
  ...
  let _role_permission = await threeaModel.findOne({
    group: reqData["upgrade"] ?? "creator",
  });
  if (_role_permission) {
  ...
  try {
    return await profileModel
            ...
            .then((_rolePermisions) => {
              if (_rolePermisions) {
                ...
                const token = jwt.sign(
                  { userID: modelData["id"], acckey: _rolePermisions["id"] },
      ...
      .catch((modelError) => {
        return res.status(404).send(modelError);
      }); // return modelError
  } catch (error) {
...
router.put(modelIName, authenticate, async (req, res) => {
  ...
  reqData = req.body ?? {};
  if (Object.keys(reqData).length == 0 && (reqData[updateKey] ?? true)) {
