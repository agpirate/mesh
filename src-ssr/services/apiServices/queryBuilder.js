...
export async function _getdeleteParams(reqParams = {}) {
  ...
  //-------------------
  let findBy = Object.keys(queryParam).length
    ? await _queryBuilder(queryParam)
    : [{}];
  //------------------
...
async function _queryBuilder(params = {}) {
  ...
  let queryNames = Object.keys(params);
  console.log("\n Incoming Query \n", params);
  if (params && queryNames.length) {
    ...
    for (let paramKey in params) {
      ...
      if (keySerialize == "qweight") {
        ...
        if (trendeVal == 3) {
          //show searched(catagories content from butttons)
          searchQueries = [
            { catagoryScore: cataVal == 1 ? { $gte: cataVal - 1 } : cataVal }, //ranging and exact searching
            { usageScore: usageVal == 1 ? { $gte: usageVal - 1 } : usageVal },
          ];
          queryNames.includes("lat")
            ? searchQueries.push({
                lat: {
                  $regex: parseFloat(String(params["lat"]).slice(0, 7)),
                  $options: "i",
                },
              })
            : "";
          queryNames.includes("long")
            ? searchQueries.push({
                long: {
                  $regex: parseFloat(String(params["long"]).slice(0, 7)),
                  $options: "i",
                },
              })
            : "";
          ...
          if (trendeVal == 2) {
            ...
            searchQueries = [
              {
                catagoryScore: {
                  $gte: cataVal - cataVal / 2,
                  $lte: cataVal + cataVal / 2,
                },
              },
              {
                usageScore: {
                  $gte: usageVal - usageVal / 2,
                  $lte: usageVal + usageVal / 2,
                },
              },
            ]; //,{Qwt:{$lt:1000000}}];
          } else {
