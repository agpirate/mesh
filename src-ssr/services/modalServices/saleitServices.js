...
async function createSaleit(
  ...
  try {
    ...
    if (reqData[createKey] ?? false) {
      findBy["_" + createKey] = new ObjectId(reqData[createKey]); //updating by id
    } else {
      //creatNew
    ...
    return await saleitModel
      ...
      .then(async (modelQA) => {
        ...
        console.log(
          ...
          "\n subOps = ",
          columnsubOperation,
          "UpdatingOperations Ends Succefully of UserID",
          reqData.userID ?? false
        );
        if (modelQA == null) {
          //==============Computing New Service -001 (for New)
          if (columnOperation) {
            //isRequired Computing Operations
            computed_ModalQA = await _primaryOperations(
              {},
              reqData,
              columnOperation,
              columnsubOperation
            ...
            console.log(
              ...
              "\n subOps = ",
              columnsubOperation,
              "PrimaryOperations- New "
            );
            if (computed_ModalQA.status != 200) {
              return computed_ModalQA;
            }

            modelQA = new saleitModel(computed_ModalQA.data);
          } else {
            modelQA = new saleitModel(reqData);
          }
        } else {
          //==============Computing Existing Service -002 (for Existing)
          if (columnOperation) {
            //isRequired Computing Operations
            let computed_ModalQA = await _primaryOperations(
              modelQA,
              reqData,
              columnOperation,
              columnsubOperation
        ...
        console.log(
          ...
          "\n subOps = ",
          columnsubOperation,
          "UpdatingOperations Ends Succefully"
        );
        //--------Computed ModelData
          ...
          .then((modelData) => {
            ...
            if (modelData && Object.keys(modelData).length) {
              return {
                status: 200,
                data: modelData,
                foreignData: computed_ModalQA.foreignData ?? { status: 401 },
              };
            } else {
              return { status: 501, data: modelData };
            }
          })
          .catch((e) => {
...
async function updateSaleit(
  ...
  try {
    //------------Preparing finding Query
    let findBy = {};
    if (reqData[updateKey] ?? false) {
      findBy["_" + updateKey] = new ObjectId(reqData[updateKey]); //updating by id
    } else {
      findBy = _nullKey; //Existing_columns(for Null O/P)
      //return res.status(404).send({ message: "NullData(P) Received." });
    }
    ...
    return await saleitModel
      ...
      .then(async (modelQA) => {
        // let columnOperation = reqData['thisOps'] ?? true
        console.log(
          "\n-------------------New UpdateSaleit--------------------------\n"
        );
        console.log(
        ...
        if (modelQA == null) {
          ...
          if (columnOperation) {
            ...
            console.log(
              "_primaryOperations UpdateSale ==<",
              computed_ModalQA.status,
              "\n"
            );
            if (computed_ModalQA.status != 200) {
              return computed_ModalQA;
