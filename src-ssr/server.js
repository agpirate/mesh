...
*/
 ...
 headers to improve load times and reduce server load.    */
  app.use(
    compression()
  ); /*Using compression middleware can reduce the size of the response body, thereby increasing 
...
*/
  ...
  app.use("/permissionapi", rolePermissionApi); //routes to be run on first to check for user token(authorization)
  //rolepermissions
  ...
  app.use("/saleitClientapi", authenticate, saleitClientApi); //routes to be run on first to check for user token(authorization)
  //--------Chats
