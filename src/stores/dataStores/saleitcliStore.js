...
const API_URL = process.env.API_IP_PORT+"/saleitcliapi"; //const API_URL = `${import.meta.env.API_URL}/users`;
...
const _suburl="/saleitcli"
const STORE_NAME ="saleitcliStore";
...
async function responseHandler(response) { ////((((((((RESPONSE HANDLER  [.data / .statusText]))))))))
  try {
    ...
    console.log('SaleitClient---------Store@',response.status,response)
...
async function requestHeader(url='') {
  ...
  _reqHeaders['_issModal']='saleitcli'
...
export const saleitcliStore = defineStore(STORE_NAME, () => {
  //state
  let Datas = ref([]); //
  let DatasDebuger = ref('');
  ...
  const getDatasDebuger = computed(() => DatasDebuger.value  );
...
async function set_syncLock(locker) {
  ...
  async function asyncDatas ()  { 
         //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
         if(syncLock.value){resultFounded.value=true;return true}
         console.log('searching.......syncStore')
          let paramObj = {}
          ...
          try {
            return await modalApi.get(process.env.API_IP_PORT+"/saleitcliapi/saleitcli",requestOptions).then((response)=>{
              if(response.status == 200) //array_Value Existed
              { 
                resultFounded.value=true 
  
                let _respData = response.data;
                let _diff = Datas.value.length  - _respData.length //has different in numbers
                //console.log('rows sync00',_diff,Datas.value[0]?.updatedAt !== _respData[0].updatedAt )
                if (_diff || ((Datas.value[0]?.updatedAt ?? false )!== (_respData[0]?.updatedAt ?? false))){//if has lengthDifference or firstIndex_Different push it
                  _newsChecker = _respData[0]
                   Datas.value = _respData;
                   return true
                } else { } 
              }
              else{ }//if no_data push as empty array ()
              return true
            }).catch((error)=>{resultFounded.value=false;Datas.value =[]
              return true}) 
          } catch (error) {    }
