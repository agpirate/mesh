...
const STORE_NAME ="publicchatStore";
...
function request(method) {
  return async (url, body = null, paramObj = {}) => {
   let _modelName =url.split('/')[1]
    ...
    requestOptions.params["Model"]  = _modelName;
    ...
    if (body != null) {
      ...
      try {
        ...
        return await responseHandler(response).then((HandledRESP) => { return{status:true,data:HandledRESP} }, // Handling Friendly Errors of the response
                                                (e) => {console.log(e);return {status:false,data:e}})
                                              .catch((e)=>{console.log(e);return {status:false,data:e}})
      } catch (e) {
      ...
      try {
        ...
        return await responseHandler(response).then((HandledRESP) => { return{status:true,data:HandledRESP} }, // Handling Friendly Errors of the response
                                                  (e) => {console.log(e);return {status:false,data:e}}) // the friendly errors
                                                  .catch((e)=>{console.log(e);return {status:false,data:e}})
      } catch (e) {
        console.log('API Response Error',e)
        return {status:false,data:e};
...
async function requestHeader(_model='') {
  //Inject Header
  console.log('\n Inject Headers Value as \n Model Name =',_model ?? 'No Model Name')
...
export const publicchatStore = defineStore(STORE_NAME, () => {
  //state
  let Datas = ref([]); //
  let status_Rows = ref('');
...
async function asyncDatas ()  { 
  ...
  if(syncLock.value){ // Allow(false) or Block(true) Syncing
   ...
   var requestOptions = {method:'GET',headers: await requestHeader('saleit'), params: {}, }; 
    ...
    return await modalApi.get(API_URL+_suburl,requestOptions).then((response)=>{
      ...
      { 
        if (response.data.length ?? false ){//|| (Datas.value[0].updatedAt!== _respData[0].updatedAt)){//if has lengthDifference or firstIndex_Different push it
          ...
          else if(status_Rows.value != 'noupdates'){ } //incoming same data  //status_Rows.value=noupdates

        } else if(status_Rows.value != null) {} //no result  //status_Rows.value=null
      }
      else {status_Rows.value = httpStatusCodes[response.status] || 'Unknown Status Code';} //no result
      return true
    }).catch((e)=>{status_Rows.value= httpStatusCodes[e.response.status] || 'Unknown Status Code' ;//null founded
      return true}) 
  } catch (e) { status_Rows.value= "BadConnection"}
  return true
};
   ...
   async function createData(formData = {}, objParam = {}) {
    ...
    try {
      return await procApiWrap
        ...
        .catch((e) => {return {status:false,data:e}; }); //Handler NonFriendly Errors
    } catch (e) {return {status:false,data:e}} //Handler Supper Non_Friendly Errors
  }
